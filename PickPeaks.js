...
pickPeaks([1,2,3,6,4,1,2,3,2,1]).showInfo();
// Expected: {pos:[3,7], peaks:[6,3]}
pickPeaks([3,2,3,6,4,1,2,3,2,1,2,3]).showInfo();
// Expected: {pos:[3,7,10], peaks:[6,3,2]}
pickPeaks([3,2,3,6,4,1,2,3,2,1,2,2,2,1]).showInfo();
// Expected: {pos:[2,4], peaks:[3,2]}
pickPeaks([2,1,3,1,2,2,2,2,1]).showInfo();
// Expected: {pos:[2], peaks:[3]}
pickPeaks([2,1,3,1,2,2,2,2]).showInfo();
// Expected: {pos:[2], peaks:[3]}
pickPeaks([2,1,3,2,2,2,2,5,6]).showInfo();
// Expected: {pos:[2], peaks:[3]}
pickPeaks([2,1,3,2,2,2,2,1]).showInfo();
// // Expected: {pos:[2,7,14,20], peaks:[5,6,5,5]}
pickPeaks([1,2,5,4,3,2,3,6,4,1,2,3,3,4,5,3,2,1,2,3,5,5,4,3]).showInfo();
// Expected: {pos:[],peaks:[]}
pickPeaks([]).showInfo();
// Expected: {pos:[],peaks:[]}
pickPeaks([1,1,1,1]).showInfo();
...
function pickPeaks(arr) {
    ...
    for (let i = 1; i < arr.length - 1; i++) {
        ...
        // Проверка, того, является ли повторяющийся элемент экстремумом
        if (arr[i] == arr[i + 1]) {
